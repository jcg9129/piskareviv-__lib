...
class NTT {
   ...
   public:
    ...
    void expand_ntt(int lg, u32* ptr) const {
        const auto mt = this->mt;
        const auto mts = this->mts;

        u32 w = mt.power<false, true>(pr_root, mod - 1 >> lg + 1);
        u32x8 a;
        for (int i = 0; i < 8; i++) {
            a[i] = mt.power<true, true>(w, i);
        }
        u32x8 dt = (u32x8)_mm256_set1_epi32(mt.power<1, 1>(w, 8));

        std::copy(ptr, ptr + (1 << lg), ptr + (1 << lg));
        transform_inverse(lg, ptr + (1 << lg));

        for (int i = 0; i < (1 << lg); i += 8) {
            store_u32x8(ptr + (1 << lg) + i, mts.mul_u32x8<false>(load_u32x8(ptr + (1 << lg) + i), a));
            a = mts.mul_u32x8<false, true>(a, dt);
        }
        transform_forward(lg, ptr + (1 << lg));

        for (int i = 0; i < (1 << lg); i += 8) {
            store_u32x8(ptr + (1 << lg) + i, mts.shrink(mts.shrink2(load_u32x8(ptr + (1 << lg) + i))));
        }
    }
};
...
namespace polynomial {
    ...
    struct aligned_allocator {
        ...
        void deallocate(T* p, std::size_t n) noexcept { _mm_free(p); }
    };

    template <class T, size_t A, class U, size_t B>
    constexpr bool operator==(const aligned_allocator<T, A>&, const aligned_allocator<U, B>&) { return std::is_same_v<T, U> && A == B; }

    template <u32 mod>
    class Poly : public std::vector<u32, aligned_allocator<u32, 32>> {
       public:
        using base = std::vector<u32, aligned_allocator<u32, 32>>;
        using base::base, base::size, base::resize;
        // using mint = MintT<mod>;

       private:
        static const NTT ntt;
        static const Montgomery mt;
        static const Montgomery_simd mts;

       public:
        u32 coeff(size_t ind) const {
            return ind < this->size() ? this->operator[](ind) : 0;
        }
        ...
        friend Poly operator*(const Poly& a, const Poly& b) {
            ...
            int s = n + m;
            int lg = (s == 0) ? 0 : 32 - __builtin_clz(s);
            lg = std::max(3, lg);
            if (n <= 1 || m <= 1 || n * m <= int64_t(1 << lg) * lg) {
                Poly c(s + 1);
                for (int i = 0; i <= n; i++) {
                    for (int j = 0; j <= m; j++) {
                        c[i + j] = mt.shrink(c[i + j] + mt.mul(a[i], b[j]));
                    }
                }
                for (int i = 0; i <= s; i++) {
                    c[i] = mt.mul(c[i], mt.r2);
                }
                return c;
            }

            Poly c(1 << lg), d(1 << lg);
            std::copy(a.begin(), a.begin() + n + 1, c.begin());
            std::copy(b.begin(), b.begin() + m + 1, d.begin());

            ntt.convolve_cyclic(lg, c.data(), d.data());
        ...
        Poly& operator+=(const Poly& b) {
            ...
            for (int i = 0; i < b.size(); i++) {
                this->operator[](i) = mt.shrink(this->operator[](i) + b[i]);
            }
            remove_zeros();
            return *this;
        }
        Poly& operator-=(const Poly& b) {
            resize(std::max(size(), b.size()));
            for (int i = 0; i < b.size(); i++) {
                this->operator[](i) = mt.shrink(this->operator[](i) + (mod - b[i]));
            }
            remove_zeros();
            return *this;
        }
        ...
        // sub  x = ax
        Poly sub_ax(u32 a) const {
            u32 p = 1;
            Poly res = *this;
            for (int i = 0; i < size(); i++, p = mt.mul(p, a)) {
                // res[i] *= p;
                res[i] = mt.mul(res[i], p);
            }
            return res;
        }
        ...
        Poly inv_series(int n) const {
            assert(n > 0);
            Poly b = {mt.power(coeff(0), mod - 2)};
            Poly c(std::bit_ceil<size_t>(n) * 2);
            for (int k = 0; (1 << k) < n; k++) {
                int lg = std::max(3, k + 2);

                // Poly c = a.mod_xk(1 << k + 1);
                c.resize(1 << k + 1);
                for (int i = 0; i < std::min<int>(1 << k + 1, size()); i++) {
                    c[i] = this->operator[](i);
                }
                ...
                ntt.transform_forward(lg, b.data());
                ntt.transform_forward(lg, c.data());

                // for (int i = 0; i < (4 * m); i++) {
                //     // b[i] *= mint(2) - b[i] * c[i];
                // }

                ntt.aux_dot_mod(lg, b.data(), c.data(), c.data());
                for (int i = 0; i < (1 << lg); i++) {
                    c[i] = mt.shrink2(2 * mod - c[i]);
                }
                for (int i = 0, stride = 1 << ntt.get_low_lg(lg); i < (1 << lg); i += stride) {
                    c[i] = mt.shrink2(2 + c[i]);
                }
                ntt.aux_dot_mod(lg, b.data(), c.data(), b.data());

                ntt.transform_inverse(lg, b.data());

                b.resize(1 << k + 1);
            }
            b.resize(n);

            // Poly r = (*this * b).mod_xk(n);
            // r.remove_zeros();
            // assert(r == Poly{1});
        ...
        u32 dot(const Poly& b) const {
            u32 res = 0;
            for (size_t i = 0; i < std::min(size(), b.size()); i++) {
                res = mt.shrink(res + mt.mul(this->operator[](i), b[i]));
            }
            res = mt.mul(res, mt.r2);
            return res;
        }
    ...
    template <u32 mod>
    const NTT Poly<mod>::ntt(mod);

    template <u32 mod>
    const Montgomery Poly<mod>::mt(mod);

    template <u32 mod>
    const Montgomery_simd Poly<mod>::mts(mod);

    // https://arxiv.org/abs/2008.08822
    template <u32 mod>
    u32 bostan_mori(u64 k, Poly<mod> p, Poly<mod> q) {
        assert(q.coeff(0) != 0);
        ...
        if (n < k) {
            ...
            ntt.transform_forward(lg, q.data());
            ntt.transform_forward(lg, p.data());
            ...
            ntt.transform_inverse(lg, q.data());
            ntt.transform_inverse(lg, p.data());
    ...
    template <u32 mod>
    u32 kth_linear(u64 k, const Poly<mod>& gen, const Poly<mod>& ch) {
